...
- **infra/postgres/migrations** — SQL-миграции Flyway (роли, схема `llm`); при `docker compose up` сервис `flyway` накатывает их после старта Postgres.
...
- **Postgres 16** — порт 5432, БД `llm_gate`; схема `llm` создаётся миграциями Flyway (`infra/postgres/migrations`).
...
Пользователи (dev): `llm_gate_admin`, `llm_gate_service`, `llm_gate_readonly` (пароли в `infra/postgres/migrations/V1__app_roles_and_users.sql`).
